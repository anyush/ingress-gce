...
func TestReportSyncStatus(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:    "ReportSyncStatus on success sets NEGsAttached condition to True",
			syncErr: nil,
			expectedConditions: []negbindingv1beta1.Condition{
				{
					Type:    NEGsAttached,
		...
		{
			desc:    "ReportSyncStatus on failure sets NEGsAttached condition to False containing error",
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			_, err := h.ReportSyncStatus(tc.syncErr)
			if err != nil {
